...
NewsController = function(mongoose) {
  ...
  base.showList = function(req, res) {
      ...
      this.getList(0, function(err, docs, main) {
          res.render(self.viewPath + 'frontlist.jade', {docs: docs, main: main});
      });
  };
  ...
  base.getMain = function(next) {
    this.Collection.find({isMain: 'on'}).sort('-createdAt').exec(function(err, main) {
      next(err, main[0]);
    });
  }
